...
python tests/run_all.py              # everything (64 test files + i18n check): parallel (4 workers), results table + single exit code (0 ⇔ all green)
...
tests/                       # 64 × test_*.py without pytest + _common.py harness + run_all.py (parallel runner) + check_i18n_keys.py — map: tests/INDEX.md
```
...
- SFTP tab in the terminal window: files over the same SSH connection — listing/".." navigation, upload/download (incl. drag & drop of files from Explorer into the tab) with progress in the status bar and cancel
