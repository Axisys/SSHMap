...
Границы undo (по спецификации v0.8.3): изменения статусов узлов (v0.7.1),
координаты при загрузке проекта и результаты автосбора (v0.9) в стек НЕ входят;
не входят также перемещение/resize групп (v0.8.1) — они остаются только
dirty-маркером (легковесные контейнеры-подписи).
