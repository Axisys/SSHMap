...
Часть сьюта, разбитого из smoke_test.py v0.6–v0.9.2 (см. INDEX.md).
  ...
  * версия формата JSON = 0.9 (единая точка истины version.py), APP_VERSION 1.2.x (с v1.2; раньше 1.1.x с v1.1, 1.0.x с v1.0RC1);
...
# Версия формата JSON — 0.9 (единая точка истины version.py); APP_VERSION — серия v1.2
# (1.0 выполнен в v1.0RC1: Терминал v1; 1.1 — диалог настроек; 1.2 — рефактор
# TerminalSessionPage «окно → страница»; формат JSON НЕ меняется)
...
check("APP_VERSION is 1.2.x (v1.2+)", getattr(_ver_mod, "APP_VERSION", "").startswith("1.2"),
