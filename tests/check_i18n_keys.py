...
Two parts (both must be clean for exit code 0):
  ...
  1. PARITY (v1.3.3): auto-discovery of EVERY i18n/*.json (the file name is the
     ...
     The meta key "name" (the language name in its own language) is NOT a
     translation: it is skipped here — a missing one only degrades the display to
     the code, it does not break the language.

  2. USED KEYS (as before): every t('key') in the code must exist in en — and, by
...
from _common import (EXPECTED_I18N_KEYS, I18N_REFERENCE, i18n_parity_problems,  # noqa: E402
...
def main(root=None):
    ...
    if not problems:
        print(f"    OK — every language covers 100% of {I18N_REFERENCE} "
              f"({EXPECTED_I18N_KEYS} translation keys; the meta key \"name\" is not counted)")

    # ── 2. The keys used in the code must exist in every discovered language ──
    ...
    problems_total = len(problems) + missing_total
    print(f"\ntotal defects: {problems_total} (parity: {len(problems)}, used keys: {missing_total})")
