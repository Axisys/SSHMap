...
class ServerNode(QGraphicsItemGroup):
    ...
    ICON_SCALE = 0.70
    # v0.8.4: вертикальная центровка точек в свёрнутой строке (подгонка по скриншоту:
    # подняты на ~11 px и сдвинуты влево на 12 px, чтобы не налезали на шеврон).
    COLLAPSED_DOT_Y = 5.0
    COLLAPSED_DOT_DX = -12.0  # сдвиг точек влево относительно развёрнутой позиции
    # Свёрнутая строка: иконка приподнята, чтобы визуально центрироваться с текстом.
    COLLAPSED_ICON_DY = -6.0
